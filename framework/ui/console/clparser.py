...
class CLParser:
    ...
    def _check_options(self, optsd):
        ...
        if "--filter" in optsd.keys() and filter(lambda x: x in optsd.keys(),
                                                 ["--sc", "--ss", "--sh", "--sl", "--sw", "--hc", "--hs", "--hh",
                                                  "--hl", "--hw"]):
            raise FuzzException(FuzzException.FATAL,
                                "Bad usage: Advanced and filter flags are mutually exclusive. Only one could be specified.")

        # Check for repeated flags
        l = ["--hc", "--hw", "--hl", "--hh", "--hs", "--sc", "--sw", "--sl", "--sh", "--ss", "--script",
             "--script-args"]
        ...
        if "--script" in optsd.keys() and "-A" in optsd.keys():
            raise FuzzException(FuzzException.FATAL,
                                "Bad usage: --scripts and -A are incompatible options, -A already defines --script=default.")
    ...
    def _parse_filters(self, optsd):
        ...
        if filter_params['regex_show'] is not None or filter_params['codes_show'] is not None or filter_params[
            'filter_string'] != "":
            ...
            for i in optsd["-z"]:
                ...
                if len(vals) == 3:
                    ...
                    for i in encoding.split("-"):
                        if i.find('@') > 0:
                            l.append(plugins.encoders.pencoder_multiple(
                                [Facade().get_encoder(ii) for ii in i.split("@")]).encode)
                        else:
